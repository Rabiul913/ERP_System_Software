            ...
            @foreach($postOffices as $key => $data)
                <tr>
                    ...
                    <td>
                        <div class="icon-btn">
                            <nobr>
                                <a href="{{ route('post-offices.edit', $data->id) }}" data-toggle="tooltip" title="Edit" class="btn btn-outline-warning"><i class="fas fa-pen"></i></a>
                                <form action="{{ url("hr/post-offices/$data->id") }}" method="POST" data-toggle="tooltip" title="Delete" class="d-inline">
                                    @csrf
                                    @method('DELETE')
