...
@section('title', 'Delivery Sales Collections Report')
...
Create
@endif
Delivery Sales Collections Report
@endsection
...
{{-- <a class="btn btn-out-dashed btn-sm btn-warning" href="{{ route('delivery-challans.index') }}"><i class="fas fa-database"></i></a> --}}
@endsection
...
@section('content')
@if ($formType == 'delivery-sales-collection')
{!! Form::open([
'url' => 'sales/delivery-sales-collection/report',
'method' => 'POST',
'class' => 'custom-form',
'files' => true,
...
<div class="row">
    ...
    <div class="col-md-4 col-sm-12">
        <div class="input-group input-group-sm input-group-primary">
            <label class="input-group-addon" for="customer_id">Customer</label>
    ...
    <div class="col-md-4 col-sm-12">
        <div class="input-group input-group-sm input-group-primary">
            <label class="input-group-addon" for="date">Category</label>
            <select name="category_id" id="category_id" class="form-control " placeholder="Select Category" required>
                <option value="" selected>All</option>
                @foreach($categories as $typeId => $category1)
                <optgroup label="{{ $category1[0]->type_name }}">
                    @foreach($category1 as $category)
                    <option value="{{ $category->id }}" {{ (old('category_id') && old('category_id')) == $category->id ? 'selected' : (!empty($subCategory->category_id) && $subCategory->category_id == $category->id ? 'selected' : '') }}>{{ $category->category_name }}</option>
                    @endforeach
                </optgroup>
                @endforeach
            </select>
    ...
    <div class="col-md-4 col-sm-12">
        <div class="input-group input-group-sm input-group-primary">
            <label class="input-group-addon" for="product_id">Product</label>
            {{-- {{Form::select('product_id', [], old('product_id'),['class' => 'form-control ','id' => 'product_id', 'placeholder'=>"All", 'autocomplete'=>"off", ""])}} --}}
            <select name="product_id" class="form-control product_id " id="product_id" autocomplete="off">
    ...
    <div class="col-md-4 col-sm-12">
        <div class="input-group input-group-sm input-group-primary">
            <label class="input-group-addon" for="zone_id">Zone</label>
    ...
    <div class="col-md-4 col-sm-12">
        <div class="input-group input-group-sm input-group-primary">
            <label class="input-group-addon" for="employee_id">Executive</label>
