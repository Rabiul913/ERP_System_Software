            ...
            <table id="customers">
                ...
                <tbody>
                    ...
                    @foreach ($reportData as $key => $data)
                        <tr>
                            <td rowspan={{  $data->deliveryOrderDetails->count() }} class="text-center">{{  date('d-M-Y', strtotime($data->date)) }}</td>
                            <td rowspan={{  $data->deliveryOrderDetails->count() }}>{{ $data->customer?->name }}</td>
                            <td rowspan={{  $data->deliveryOrderDetails->count() }}>{{ $data->employee?->emp_name }}</td>
                            <td rowspan={{ $data->deliveryOrderDetails->count()  }} class="text-center">DO-{{ $data->id }}</td>
                            <td rowspan={{ $data->deliveryOrderDetails->count() }} class="text-center">{{ $data->zone?->zone }}</td>
                            @if(count($data->deliveryOrderDetails))
                                <td>{{ $data->deliveryOrderDetails[0]->product?->category?->name }}</td>
                                <td>{{ $data->deliveryOrderDetails[0]->product?->name }}</td>

                                <td class="text-center">{{ $data->deliveryOrderDetails[0]->quantity }}</td>
                                <td class="text-right">@money( $data->deliveryOrderDetails[0]->unit_price )</td>
                            @endif
                            {{-- <td rowspan={{ $data->deliveryOrderDetails->count() }}>{{ $data->delivery_address }}</td> --}}
                            {{-- <td rowspan={{ $data->deliveryOrderDetails->count() }} class="text-right">{{ $data->labor_cost }}</td>
                            <td rowspan={{ $data->deliveryOrderDetails->count() }} class="text-right">{{ $data->rent }}</td> --}}
                            {{-- <td rowspan={{ $data->deliveryOrderDetails->count() }} class="text-right">{{ $data->discount }}</td> --}}
                            <td rowspan={{ $data->deliveryOrderDetails->count() }} class="text-right">@money( $data->total )</td>
                            {{-- <td></td> --}}
                            <td rowspan={{ $data->deliveryOrderDetails->count() }} class="text-right">@money( $data->paid )</td>
                            <td rowspan={{ $data->deliveryOrderDetails->count() }} class="text-right">@money( $data->due )</td>
                        </tr>
                ...
                <tfoot>
                    <tr>
                        <td colspan="9" class="text-right" style="width: 90%; font-weight:bold;">Total</td>
