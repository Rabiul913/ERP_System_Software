        ...
        <div style="width: 50%; float:left;">
            <div style="margin-top: 20px;">
                ...
                <p style="font-size: 12px; text-align: center; font-weight: bold;text-transform: uppercase; ">
                    {{ $search_criteria["Product"] != "All" ? "Product Wise " : ($search_criteria["Category"] != "All" ? "Category Wise " : '') }}Sales Return Report ({{ $search_criteria["Product"] != "All" ? $search_criteria["Product"] : ($search_criteria["Category"] != "All" ? $search_criteria["Category"] : '') }})
...
<div >
    ...
    @if(count($reportData))
            ...
            <div>
                ...
                <div>
                    <table class="customers">
                        ...
                        <tbody>
                            @foreach ($data as $salesReturnDetail)
                            <tr>
                                {{-- dd() --}}
                                <td style="width: 12%;" class="text-center">{{ date('d-M-Y', strtotime($salesReturnDetail->sales_return?->return_date)) }}</td>
                                <td style="width: 15%;" class="">{{ $salesReturnDetail->sales_return?->delivery_challan?->customer?->name }}</td>
                                <td style="width: 15%;" class="">{{ $salesReturnDetail->sales_return?->delivery_challan?->deliveryOrder?->employee?->emp_name }}</td>
                                <td style="width: 12%;" class="text-center">DO-{{ $salesReturnDetail->sales_return?->delivery_challan?->deliveryOrder?->id }}</td>
                                <td style="width: 12%;" class="text-center">{{ $salesReturnDetail->sales_return?->delivery_challan?->deliveryOrder?->zone?->zone }}</td>

                                <td style="width: 12%;" class="text-center">{{ $salesReturnDetail->quantity }}</td>
                                <td style="width: 12%;" class="text-right">@money( $salesReturnDetail->return_price )</td>
                                <td style="width: 10%;" class="text-right">@money( $salesReturnDetail->quantity * $salesReturnDetail->return_price )</td>
                                @php
                                    $sub_total += ($salesReturnDetail->quantity * $salesReturnDetail->return_price);
                                @endphp
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="7" class="text-right" style="width: 90%; font-weight:bold;">Sub-total</td>
        ...
        <table class="customers" style="margin-top: 10px;">
            ...
            <tbody>
                <tr>
                    <td colspan="7" class="text-right" style="width: 90%; font-weight:bold;">Grand-total</td>
