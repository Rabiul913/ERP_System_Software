            ...
            <table id="customers">
                ...
                <tbody>
                    ...
                    @foreach ($reportData as $key => $data)
                        <tr>
                            <td rowspan={{  $data->sales_return_details->count() }} class="text-center">{{  date('d-M-Y', strtotime($data->return_date)) }}</td>
                            <td rowspan={{  $data->sales_return_details->count() }}>{{ $data->delivery_challan?->customer?->name }}</td>
                            <td rowspan={{  $data->sales_return_details->count() }}>{{ $data->delivery_challan?->deliveryOrder?->employee?->emp_name }}</td>
                            <td rowspan={{ $data->sales_return_details->count()  }} class="text-center">DO-{{ $data->delivery_challan?->delivery_order_id }}</td>
                            <td rowspan={{ $data->sales_return_details->count() }} class="text-center">{{ $data->delivery_challan?->deliveryOrder?->zone->zone }}</td>
                            @if(count($data->sales_return_details))
                                <td>{{ $data->sales_return_details[0]->product?->category?->name }}</td>
                                <td>{{ $data->sales_return_details[0]->product?->name }}</td>

                                <td class="text-center">{{ $data->sales_return_details[0]->quantity }}</td>
                                <td class="text-right">@money( $data->sales_return_details[0]->return_price )</td>
                            @endif

                            @php
                                $total_return_price = collect($data->sales_return_details)->sum(function ($detail) {
                                    return $detail->quantity * $detail->return_price;
                                });

                                $total += $total_return_price;
                            @endphp
                            <td rowspan={{ $data->sales_return_details->count() }} class="text-right">@money( $total_return_price )</td>
                            {{-- <td rowspan={{ $data->sales_return_details->count() }} class="text-right">{{ $data->paid }}</td>
                            <td rowspan={{ $data->sales_return_details->count() }} class="text-right">{{ $data->due }}</td> --}}
                        </tr>
                        @foreach ($data->sales_return_details as $key2=>$item)
                        @if($key2 == 0) @continue @endif
                                                    <tr>
                                                        ...
                                                        <td class="text-right">@money( $item->return_price )</td>
                                                    </tr>
                        @endforeach
                    @endforeach
                ...
                <tfoot>
                    <tr>
                        <td class="text-right" colspan="9" style="font-weight:bold;">Total</td>
